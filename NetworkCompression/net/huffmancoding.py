            ...
            if 'conv' in name:
                ...
                for i in range(shape[0]):
                    for j in range(shape[1]):
                        data = weight[i][j]
                        mat = csr_matrix(data)
                        data_array.append(mat.data)
                        len_array.append(len(mat.data))
                        indices_array.append(mat.indices)
                        indptr_array.append(calc_index_diff(mat.indptr))
                #print(mat.indptr)
                data_array = np.concatenate(data_array)
                indices_array = np.concatenate(indices_array)
                indptr_array = np.concatenate(indptr_array)
                len_array = np.array(len_array,dtype=np.int32)
                
                t0, d0 = huffman_encode(data_array, name+f'_{form}_data', directory)
                t1, d1 = huffman_encode(len_array, name+f'_{form}_len_data', directory)
                t2, d2 = huffman_encode(indices_array, name+f'_{form}_indices', directory)
                t3, d3 = huffman_encode(indptr_array, name+f'_{form}_indptr', directory)
    ...
    for name, param in model.named_parameters():
        if 'weight' in name:
            ...
            if 'conv' in name:
                ...
                # Decode data
                data_array = huffman_decode(directory, name+f'_{form}_data', dtype='float32')
                len_array = huffman_decode(directory, name+f'_{form}_len_data', dtype='int32')
                indices_array = huffman_decode(directory, name+f'_{form}_indices', dtype='int32')
                indptr_array = huffman_decode(directory, name+f'_{form}_indptr', dtype='int32')
                indptr_array_2 = []
                index = 0
                for i in range(shape[0]*shape[1]):
                    indptr_array_2.append(reconstruct_indptr(indptr_array[index:index+shape[3]]))
                    #print(reconstruct_indptr(indptr_array[index:index+shape[3]]))
                    index += shape[3]
                # Construct matrix
                cnt = 0
                index = 0
                ...
                for item in len_array:
                    mat = matrix((data_array[cnt:cnt+item], indices_array[cnt:cnt+item], indptr_array_2[index]),(shape[2],shape[3]))
                    index += 1
                    cnt += item
                    mat = mat.toarray()
                    mat_array.append(mat)
                mat = np.concatenate(mat_array)
                new_mat_2 = []
                new_mat_4 = []
                for i in range(shape[0]*shape[1]):
                    new_mat_1 = []
                ...
                # Insert to model
                param.data = torch.from_numpy(new_mat_4).to(dev)
